 ...
 * On **Mac OS X''', visit your '''Applications''' folder and double-click on the '''TextEdit''' app.  A blank editor window should come up.  Now, go to the '''Format''' menu and select the '''Make Plain Text** option.  Okay, you're all set! ![](/static/man-editor-osx.png)
...
All of the <span style="color:red">self</span>s in the above example are the App object.  Shoes uses Ruby's
 ...
 * <span style="color:red">top</span>: the y-coordinate for the oval pen.
...
Sets the line size for all drawing within this slot.  Whereas the <span style="color:red">stroke</span>
...
The <span style="color:red">full_height</span> method gives you the height of image (in pixels) as it was
...
Blurs the image by <span style="color:red">radius</span> amount.

### glow(radius: a number, :inner  false or true)  ###
Adds a glow to your image, either outer glow (default) or inner glow (set optional `:inner` style to true), amount of glow is set by <span style="color:red">radius</span> attribute.
...
Adds a shadow to your image, <span style="color:red">distance</span> sets the placement of the shadow along a line going from top-left corner to bottom-right corner, the biggest the attribute the more distant the shadow, blurriness of the shadow is controlled by the <span style="color:red">radius</span> attribute.

If you need more precise placement, the method offers two styles for that : `:displace_left` and `:displace_top`, if used they take precedence over the <span style="color:red">distance</span> attribute.
...
item, for instance,) its <span style="color:red">change</span> block is called. The block is given <span style="color:red">self</span>,
which is the list box object which has changed.

### choose(item: a string) » self ###

Selects the option in the list box that matches the string given by <span style="color:red">item</span>.
...
now. If nothing is selected, <span style="color:red">nil</span> will be the reply.
...
together in the same slot (along with a bunch of <span style="color:red">para</span>.)
...
Returns whether the radio button is checked or not. So, <span style="color:red">true</span> means "yes, it
is checked!"
...
When the radio button is clicked, its <span style="color:red">click</span> block is called. The block is
handed <span style="color:red">self</span>, which is an object representing the radio which was clicked.
...
so in a svg of playing cards for example you might want to draw '#diamonds_queen'
...
<span style="color:red">Note</span> the filename `is not parsed` for the extention to determine if there
is a .png at the end
...
most people don't need it. But, if you do
...
perfectly happy C `printf()` and Ruby <span style="color:red">puts</span> that don't normallly display. 

But, sometimes, a guy has to do guy things and is willing to accept the downsides: 
You can't undo the call and switch them back although a serial abuser of
defenseless stdin's will try. You know who you are. 

If you do call Shoes::show_console and Shoes thinks you're not running from a
terminal then it brings up a window that looks similar to the Shoes Log Window. 
Only it isn't. It's not pretty. No color coding. Don't even attempt to send escape
sequences to it. Which some of you will do on Windows and it might work THERE 
for YOU. Don't be that person. 

That Window can update slowly. It runs in the background for some platforms.
It can lose output on a busy system. Once started, Shoes scripts have no control of it.
NONE, NADA, ZIP. The readline gem (included with Shoes) does works if your
expecations are low. We will mock and taunt you for attempting to use curses and reporting
that it doesn't work. Don't be that person.

Still, if you need to write a Shoes script to monitor some long running process that outputs
to stdout/stderr from some other program and you want to copy the log to the clipboard
or scroll it back, now you can do that. 

No, it can't fix the horrible Windows console and you shouldn't try too either.
...
```ruby --ruby rubyargments ``` This one uses the ruby inside Shoes to execute the 
<span style="color:red">rubyargments</span> One of those arguments would be a script to execute with 
...
There's no <span style="color:red">one</span> proper way that I know of and probably six ways to Sunday to do it.
